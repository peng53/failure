...
pack [radiobutton .frame_m_bar.rep -text Repeat -variable osnv::MAC -value rep -command mac_on] -side left
pack [radiobutton .frame_m_bar.reg -text Register -variable osnv::MAC -value reg -command mac_on] -side left
...
pack [frame .frame_kb.r$r] -fill x
pack [button .frame_kb.r$r.bBackspace -text Backspace -command kbackspace] -side left
set SPACE { }
...
foreach {s} {{`-=} {[]} {;'} {,/}} {
	...
	foreach {c} [split $s {}] {
		pack [button .frame_sp.r$r.b$c -text $c -command "wri_rep \\$c"] -fill both -expand 1 -side left
	}
	incr r
}
pack [button .frame_sp.r1.bBackslash -text \\ -command {wri_rep \\}] -fill both -expand 1 -side left
pack [button .frame_sp.r3.bPeriod -text . -command {wri_rep .}] -fill both -expand 1 -side left
	...
	switch $o {
		...
		rep {
			switch $c {
				Delete {
					set osnv::s {}
					if {!$osnv::locked} { .frame_m_bar.off select }
				}
				1 - 2 - 3 - 4 - 5 - 6 - 7 - 8 - 9 - 0 {
		...
		reg {
			switch $c {
				{ } {
					.maintext.mac_words.lb insert end [.maintext.t get $osnv::s insert]
					set osnv::s {}
					if {!$osnv::locked} { .frame_m_bar.off select }
				}
				Enter {
