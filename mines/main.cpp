...
void printrf(std::vector<bool> &rf,std::vector<char> &aj,short hw, short t,short left){
	putchar('/');
	for (short i=0;i<hw;i++) printf("%d",i%10);
	printf("->left:%d",left);
	for (short i=0;i<t;++i){
...
void rfil(std::vector<char> &aj,std::vector<bool> &rf, short n,short hw,short &l){
	if (rf[n]) return; // no change
	if (aj[n]!=0){
		if (aj[n]!=-1){
			rf[n]=1;
			l--;
...
void pickc(std::vector<char> &aj,std::vector<bool> &rf, short n,short hw,short &l){
	if (rf[n]) return;
	if (aj[n]==-1){
		rf[n]=1;
		l=-1;
...
void revil(std::vector<bool> &rf, std::vector<short> &mines,short m){
...
int main(int argc,char *argv[]){
	...
	printfus(aj,hw,t);
	std::vector<bool> rf(t,0);
	printrf(rf,aj,hw,t,left);
	while (left>=1){
		printf("Enter x,y: ");
		scanf("%hd,%hd",&x,&y);
