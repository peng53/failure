...
void rmine(T_CELLS (&mines)[N_CELLS]){
	srand(time(NULL));
	T_CELLS j,t;
...
void nfields(char (&aj)[N_CELLS],T_CELLS (&mines)[N_CELLS]){
	bool tw,te;
	short n;
...
void printfus2(char (&aj)[N_CELLS]){
	putchar('/');
...
/*
void printrf(bool (&rf)[N_CELLS],char (&aj)[N_CELLS]){
	for (T_CELLS i=0;i<N_CELLS;++i){
		if (i%N_WIDTH==0) putchar('\n');
		if (rf[i]){
			switch (aj[i]){
				case -1: printf("%4s","*"); break;
				case 0: printf("%4s"," "); break;
				default: printf("%4d",aj[i]); break;
			}
		} else {
			printf("(%2d)",i);
		}
...
T_CELLS rfil(char (&aj)[N_CELLS],bool (&rf)[N_CELLS],T_CELLS n,T_CELLS left){
	if (rf[n]) return left;
...
T_CELLS pickc(char (&aj)[N_CELLS],bool (&rf)[N_CELLS],T_CELLS n,T_CELLS left){
