...
SQLi::SQLi(sqlite3* _db): db(_db){
	...
	sqlite3_prepare_v2(db,"UPDATE records SET uid=?1, code=?2, start_time=?3, end_time=?4, desc=?5 WHERE rowid=?6",-1,&upd,0);
...
int SQLi::ins_table(const Record &t){
	sqlite3_reset(ins);
...
int SQLi::upd_row(const Record &t){
	sqlite3_reset(upd);
	sqlite3_bind_text(upd,1,t.uid.c_str(),-1,SQLITE_TRANSIENT);
	sqlite3_bind_text(upd,2,t.code.c_str(),-1,SQLITE_TRANSIENT);
	sqlite3_bind_int(upd,3,t.ds);
	sqlite3_bind_int(upd,4,t.de);
	sqlite3_bind_text(upd,5,t.desc.c_str(),-1,SQLITE_TRANSIENT);
...
int SQLi::get_row(const unsigned rnum,Record &R){
	sqlite3_reset(getr);
	sqlite3_bind_int(getr,1,rnum);
	sqlite3_step(getr);
	R.rnum = rnum;
