...
DTE = {
	...
	'd': "SELECT * from prod_records WHERE date=?"
}
...
def print_rows(rows,s,c):
	...
	for d in xrange(len(rows),s+c):
		print "%2d | %6s | %16s | %16s | %5s |" %(d,'','','','')
	print '-'*82
			...
			if len(t)>0:
				...
				if s in DTE:
					try:
						...
						print e
						print "Date contains invalid characters."
				else:
					return [EXT[s],(t,)]
			else:
				print "A requirement is required."
		print "Unknown option entered. 303"
...
def main():
	...
	while True:
		...
		if s=='L':
			...
			q = drowchoose(len(N),N_S,10)
			if q:
				for n in q:
					N.pop(n)
				delta = 1
		elif s=='a':
			q = drowchoose(len(R),R_S,10)
			if q:
				for n in q:
					N.append(R[n])
				delta = 1
